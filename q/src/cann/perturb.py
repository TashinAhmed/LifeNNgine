...
from src.cann.train import train_model, seed_all, get_git_hash, BIG_PRIME
...
def perturb(**kwargs):
  ...
  epoch_index = kwargs.get("epoch_index", -1)
  repeats = kwargs.get("repeats", 1)
  base_seed = kwargs.get("pseudorandom_seed", 1)
  j_count = kwargs.get("j_zero_perturbations", 0)
  k_count = kwargs.get("k_sign_perturbations", 0)
  u_magnitude = kwargs.get("uniform_magnitude", 0.0)
  ...
  for run_index in range(len(exp_df)):
    ...
    experiment_name = f"perturb_{experiment_tag}_{run_number}"
...
if __name__ == "__main__":
  ...
  parser.add_argument("-g", "--uniform_magnitude", type=float, default=0.0,\
