...
def train_multistep(steps, device="cpu", epochs=100):
    ...
    model = PolyKAN(degree=3, width=4, depth=2).to(device) # Same model config
...
def run_experiment():
    ...
    for s in step_list:
        ...
        results[s] = acc
    with open("extended_experiments_02/results.json", "w") as f:
        json.dump(results, f, indent=2)
    steps = list(results.keys())
    accs = list(results.values())
    plt.figure(figsize=(8, 5))
    plt.plot(steps, accs, marker="o", linestyle="-")
    plt.title("Game of Life Multi-Step Prediction (Set 02)")
    plt.xlabel("Prediction Steps (n)")
    plt.ylabel("Accuracy (Valid Region)")
    plt.grid(True)
    plt.ylim(-0.1, 1.1)
    plt.xticks(step_list)
    plt.savefig("extended_experiments_02/accuracy_plot.png")
    print("Results saved to extended_experiments_02/")
