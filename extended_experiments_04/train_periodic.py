...
def get_periodic_data(engine, num_samples, grid_size, steps):
    assert engine.padding_mode == 'circular', "Engine must handle circular padding"
...
def train_periodic(steps, device='cpu', epochs=150):
    ...
    # init PolyKAN with circular padding
    # Using the standard "successful" config: Degree 3, Width 4, Depth 2
...
def run_experiment():
    ...
    for s in step_list:
        # 3 trials to avoid seed variance
        trial_accs = []
        for t in range(3):
    ...
    with open('extended_experiments_04/results.json', 'w') as f:
    ...
    plt.plot(steps, accs, marker='o', linestyle='-', label='Periodic (No Crop)')
    plt.title("Game of Life Multi-Step Prediction (Periodic Boundaries)")
    ...
    plt.legend()
    plt.savefig('extended_experiments_04/accuracy_plot.png')
    print("Results saved to extended_experiments_04/")
