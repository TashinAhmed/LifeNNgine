...
def train_and_eval(model, X_train, y_train, X_val, y_val, steps, epochs=200):
...
def run_search():
    ...
    print("Starting Architecture Search for >95% Accuracy (Periodic)...")
    ...
    for s in steps_to_solve:
        ...
        for deg, width, depth in search_space:
            ...
            for t in range(2):
                ...
                model = PolyKAN(degree=deg, width=width, depth=depth, padding_mode='circular').to(device)
    ...
    with open('EE2/search/results.json', 'w') as f:
